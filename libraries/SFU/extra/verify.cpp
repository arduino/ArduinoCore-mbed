...
int verify(FILE* update_file) {
  ...
  if (ota_header.header.magic_number != 0x2341005E) /* 2341:005E = VID/PID Nano RP2040 Connect */
  {
    printf("Wrong magic\n");
    return -3;
  }
...
int decompress(FILE* update_file) {
...
int decompress_and_flash(FILE* update_file, FlashIAP* flash, size_t offset) {
  ...
  flash->init();
  // erase the entire 1MB after SFU
  flash->erase(XIP_BASE + offset, (1024*1024 - offset));

  uint32_t const LZSS_FILE_SIZE = update_file_size - sizeof(ota_header.buf);

  lzss_init(update_file, flash, offset, LZSS_FILE_SIZE);
  lzss_decode();
  lzss_flush();
