...
static int anx7625_init_pdata(struct anx7625 *anx7625)
{
	...
	/* GPIO for HPD */
	pdata->gpiod_intp = devm_gpiod_get(dev, "intp", GPIOD_IN);
	if (IS_ERR(pdata->gpiod_intp))
		return PTR_ERR(pdata->gpiod_intp);
...
static irqreturn_t anx7625_cdet_threaded_handler(void *data)
{
	struct anx7625 *anx7625 = (struct anx7625 *)data;
	int connected;
...
static irqreturn_t anx7625_intp_threaded_handler(void *data)
{
	struct anx7625 *anx7625 = (struct anx7625 *)data;
	unsigned char c;
...
int anx7625_i2c_probe(struct anx7625 *anx7625)
{
	...
	pdata->cdet_irq = new mbed::InterruptIn(pdata->gpiod_cdet->name);

	pdata->intp_irq = new mbed::InterruptIn(pdata->gpiod_intp->name);
